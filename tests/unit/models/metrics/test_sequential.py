...
def test_sequential_metric_attach_train(engine: BaseEngine) -> None:
    ...
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[0].reset), EngineEvents.TRAIN_EPOCH_STARTED
    )
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[0].value, handler_kwargs={"engine": engine}),
        EngineEvents.TRAIN_EPOCH_COMPLETED,
    )
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[1].reset), EngineEvents.TRAIN_EPOCH_STARTED
    )
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[1].value, handler_kwargs={"engine": engine}),
        EngineEvents.TRAIN_EPOCH_COMPLETED,
    )
...
def test_sequential_metric_attach_eval(engine: BaseEngine) -> None:
    ...
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[0].reset), EngineEvents.EVAL_EPOCH_STARTED
    )
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[0].value, handler_kwargs={"engine": engine}),
    ...
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[1].reset), EngineEvents.EVAL_EPOCH_STARTED
    )
    assert engine.has_event_handler(
        GEventHandler(metric.metrics[1].value, handler_kwargs={"engine": engine}),
        EngineEvents.EVAL_EPOCH_COMPLETED,
