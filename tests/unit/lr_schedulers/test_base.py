...
def test_setup_lr_scheduler_optimizer_none():
...
def test_setup_lr_scheduler_lr_scheduler_none():
    optimizer = SGD(nn.Linear(4, 6).parameters(), lr=0.01)
    assert setup_lr_scheduler(optimizer=optimizer, lr_scheduler=None) is None


def test_setup_lr_scheduler_lr_scheduler_object_correct_optimizer(caplog: LogCaptureFixture):
...
def test_setup_lr_scheduler_lr_scheduler_object_incorrect_optimizer(caplog: LogCaptureFixture):
    with caplog.at_level(logging.WARNING):
...
def test_setup_lr_scheduler_lr_scheduler_dict():
    optimizer = SGD(nn.Linear(4, 6).parameters(), lr=0.01)
