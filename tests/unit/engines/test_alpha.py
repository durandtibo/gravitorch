...
def test_alpha_engine_str(core_creator: BaseCoreCreator) -> None:
...
def test_alpha_engine_iteration(core_creator: BaseCoreCreator) -> None:
    assert AlphaEngine(core_creator).iteration == -1


def test_alpha_engine_lr_scheduler(core_creator: BaseCoreCreator) -> None:
    assert AlphaEngine(core_creator).lr_scheduler is None


def test_alpha_engine_max_epochs(core_creator: BaseCoreCreator) -> None:
    assert AlphaEngine(core_creator).max_epochs == 1


def test_alpha_engine_model(core_creator: BaseCoreCreator) -> None:
    assert isinstance(AlphaEngine(core_creator).model, nn.Linear)


def test_alpha_engine_optimizer(core_creator: BaseCoreCreator) -> None:
    assert isinstance(AlphaEngine(core_creator).optimizer, Optimizer)


def test_alpha_engine_random_seed(core_creator: BaseCoreCreator) -> None:
    assert AlphaEngine(core_creator).random_seed == 9984043075503325450


def test_alpha_engine_should_terminate(core_creator: BaseCoreCreator) -> None:
    assert not AlphaEngine(core_creator).should_terminate


def test_alpha_engine_add_event_handler(core_creator: BaseCoreCreator) -> None:
...
def test_alpha_engine_add_history_with_key(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_history(MinScalarHistory("name"), "loss")
    assert engine._state.has_history("loss")


def test_alpha_engine_add_history_without_key(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_history(MinScalarHistory("loss"))
    assert engine._state.has_history("loss")


def test_alpha_engine_add_module(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_module("my_module", nn.Linear(4, 6))
    assert engine._state.has_module("my_module")


def test_alpha_engine_create_artifact(core_creator: BaseCoreCreator) -> None:
...
def test_alpha_engine_get_history_exists(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_get_history_does_not_exist(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_get_histories(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_get_histories_empty(core_creator: BaseCoreCreator) -> None:
    assert AlphaEngine(core_creator).get_histories() == {}


def test_alpha_engine_get_module_exists(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_module("my_module", nn.Linear(4, 6))
...
def test_alpha_engine_has_event_handler_true(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_event_handler("my_event", GEventHandler(increment_epoch_handler))
    assert engine.has_event_handler(GEventHandler(increment_epoch_handler), "my_event")


def test_alpha_engine_has_event_handler_false(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert not engine.has_event_handler(GEventHandler(increment_epoch_handler), "my_event")
...
def test_alpha_engine_has_history_false(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert not engine.has_history("loss")


def test_alpha_engine_has_module_true(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_module("my_module", nn.Linear(4, 6))
    assert engine.has_module("my_module")


def test_alpha_engine_has_module_false(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert not engine.has_module("my_module")


def test_alpha_engine_increment_epoch_1(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert engine.epoch == -1
...
def test_alpha_engine_increment_epoch_2(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert engine.epoch == -1
    engine.increment_epoch(2)
    assert engine.epoch == 1
...
def test_alpha_engine_increment_iteration_1(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_increment_iteration_2(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert engine.iteration == -1
...
def test_alpha_engine_load_state_dict(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_log_figure_without_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
    figure = Mock()
...
def test_alpha_engine_log_figure_with_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
    figure = Mock()
...
def test_alpha_engine_log_figures_without_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_log_figures_with_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
    figure1, figure2 = Mock(), Mock()
...
def test_alpha_engine_log_metric_without_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_log_metric_with_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_log_metrics_without_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_log_metrics_with_step(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_remove_event_handler_exists(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_event_handler(
        "my_event",
        GEventHandler(increment_epoch_handler, handler_kwargs={"engine": engine}),
...
def test_alpha_engine_remove_module_exists(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.add_module("my_module", nn.Linear(4, 6))
...
def test_alpha_engine_state_dict(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    assert set(engine.state_dict().keys()) == {"modules", "epoch", "histories", "iteration"}


def test_alpha_engine_terminate_initial_false(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
    engine.terminate()
    assert engine.should_terminate


def test_alpha_engine_terminate_initial_true(core_creator: BaseCoreCreator) -> None:
    engine = AlphaEngine(core_creator)
...
def test_alpha_engine_train_with_terminate(core_creator: BaseCoreCreator) -> None:
    def terminate_handler(engine: BaseEngine) -> None:
        if engine.epoch == 2:
            engine.terminate()
    ...
    engine.add_event_handler(
        ...
        GEventHandler(terminate_handler, handler_kwargs={"engine": engine}),
    )
    engine.train()
...
def test_alpha_engine_log_best_metrics(core_creator: BaseCoreCreator) -> None:
    exp_tracker = Mock()
    engine = AlphaEngine(core_creator, exp_tracker=exp_tracker)
...
def test_alpha_engine_setup_exp_tracker_with_none(core_creator: BaseCoreCreator) -> None:
