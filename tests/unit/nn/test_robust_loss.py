...
def test_asinh_mse_loss_forward() -> None:
...
def test_asinh_mse_loss_forward_reduction(reduction: str) -> None:
...
def test_msle_loss_str() -> None:
    assert str(MSLELoss()).startswith("MSLELoss(")


def test_msle_loss_forward() -> None:
    criterion = MSLELoss()
    loss = criterion(torch.ones(2, 3, requires_grad=True), torch.ones(2, 3))
    loss.backward()
    assert loss.equal(torch.tensor(0.0))
...
@mark.parametrize("reduction", ("mean", "sum", "none"))
def test_msle_loss_forward_reduction(reduction: str) -> None:
...
def test_symlog_mse_loss_str() -> None:
    assert str(SymlogMSELoss()).startswith("SymlogMSELoss(")


def test_symlog_mse_loss_forward() -> None:
...
@mark.parametrize("reduction", ("mean", "sum", "none"))
def test_symlog_mse_loss_forward_reduction(reduction: str) -> None:
...
def test_relative_mse_loss_eps_default() -> None:
    assert RelativeMSELoss()._eps == 1e-8


def test_relative_mse_loss_forward() -> None:
...
def test_relative_mse_loss_forward_reduction(reduction: str, eps: float) -> None:
...
def test_relative_smooth_l1_loss_beta_default() -> None:
    assert RelativeSmoothL1Loss()._beta == 1.0


def test_relative_smooth_l1_loss_eps_default() -> None:
    assert RelativeSmoothL1Loss()._eps == 1e-8


def test_relative_smooth_l1_loss_forward() -> None:
...
@mark.parametrize("eps", (1e-5, 1e-8))
def test_relative_smooth_l1_loss_forward_reduction(reduction: str, beta: float, eps: float) -> None:
...
def test_symmetric_relative_smooth_l1_loss_str() -> None:
    assert str(SymmetricRelativeSmoothL1Loss()).startswith("SymmetricRelativeSmoothL1Loss(")


def test_symmetric_relative_smooth_l1_loss_beta_default() -> None:
    assert SymmetricRelativeSmoothL1Loss()._beta == 1.0


def test_symmetric_relative_smooth_l1_loss_eps_default() -> None:
    assert SymmetricRelativeSmoothL1Loss()._eps == 1e-8


def test_symmetric_relative_smooth_l1_loss_forward() -> None:
