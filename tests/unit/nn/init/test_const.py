...
from gravitorch.nn.init import Constant, ConstantBias, constant, constant_bias
...
def test_constant_initialize_value(value: Union[int, float]) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with patch("gravitorch.nn.init.const.constant") as init:
...
def test_constant_bias_initialize_value(value: Union[int, float]) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with patch("gravitorch.nn.init.const.constant_bias") as init:
...
def test_constant_learnable_only_true() -> None:
    module = nn.Sequential(nn.Linear(4, 6), nn.Linear(6, 6))
    freeze_module(module[1])
    constant(module, 0.0, learnable_only=True)
    assert module[0].weight.data.equal(torch.zeros(6, 4))
...
def test_constant_learnable_only_false() -> None:
    module = nn.Sequential(nn.Linear(4, 6), nn.Linear(6, 6))
    freeze_module(module[1])
    constant(module, 0.0, learnable_only=False)
    assert module[0].weight.data.equal(torch.zeros(6, 4))
    assert module[0].bias.data.equal(torch.zeros(6))
...
def test_constant_log_info_true(caplog: LogCaptureFixture) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with caplog.at_level(level=logging.INFO):
        constant(module, value=0.0, log_info=True)
        assert module.weight.equal(torch.zeros(6, 4))
        assert module.bias.equal(torch.zeros(6))
        assert caplog.messages


def test_constant_log_info_false(caplog: LogCaptureFixture) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with caplog.at_level(level=logging.INFO):
        constant(module, value=0.0)
        assert module.weight.equal(torch.zeros(6, 4))
        assert module.bias.equal(torch.zeros(6))
...
###################################
#     Tests for constant_bias     #
###################################


@mark.parametrize("value", (1, 2.0))
def test_constant_bias(value: Union[int, float]) -> None:
    module = nn.Linear(4, 6)
    constant(module, 0.0)
    constant_bias(module, value=value)
    assert module.bias.equal(torch.full((6,), value, dtype=torch.float))


@mark.parametrize("module", MODULE_WITH_BIAS)
def test_constant_bias_module_with_bias(module: nn.Module) -> None:
    constant(module, 1.0)
    assert module.bias.equal(torch.ones(6))
    constant_bias(module, value=0.0)
    assert module.bias.equal(torch.zeros(6))


@mark.parametrize("module", MODULE_WITHOUT_BIAS)
def test_constant_bias_module_without_bias(module: nn.Module) -> None:
    constant_bias(module, value=0.0)
    assert module.bias is None


def test_constant_bias_module_multihead_attention() -> None:
...
def test_constant_bias_module_multihead_attention_add_bias_kv() -> None:
...
def test_constant_bias_module_multihead_attention_no_bias() -> None:
...
def test_constant_bias_learnable_only_true() -> None:
    module = nn.Sequential(nn.Linear(4, 6), nn.Linear(6, 6))
    constant(module, 1)
    freeze_module(module[1])
    constant_bias(module, 0.0, learnable_only=True)
...
def test_constant_bias_learnable_only_false() -> None:
    module = nn.Sequential(nn.Linear(4, 6), nn.Linear(6, 6))
    constant(module, 1.0)
    constant_bias(module, 0.0)
    assert module[0].weight.data.equal(torch.ones(6, 4))  # weights are not initialized
    assert module[0].bias.data.equal(torch.zeros(6))
...
def test_constant_bias_log_info_true(caplog: LogCaptureFixture) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with caplog.at_level(level=logging.INFO):
        constant_bias(module, value=0.0, log_info=True)
        assert module.bias.equal(torch.zeros(6))
        assert caplog.messages


def test_constant_bias_log_info_false(caplog: LogCaptureFixture) -> None:
    module = nn.Linear(4, 6)
    constant(module, 1.0)
    with caplog.at_level(level=logging.INFO):
        constant_bias(module, value=0.0)
