...
def test_vanilla_engine_state_str():
    assert str(VanillaEngineState()).startswith("VanillaEngineState(")


def test_vanilla_engine_state_default_values():
...
@mark.parametrize("key", NAMES)
def test_vanilla_engine_state_add_history_without_key(key: str):
    state = VanillaEngineState()
...
def test_vanilla_engine_state_add_module(name: str):
    state = VanillaEngineState()
    state.add_module(name, nn.Linear(4, 5))
    assert state._modules.has_module(name)


def test_vanilla_engine_state_get_best_values_without_history():
    assert VanillaEngineState().get_best_values() == {}


def test_vanilla_engine_state_get_best_values_with_history():
    state = VanillaEngineState()
...
def test_vanilla_engine_state_get_history_exists():
    state = VanillaEngineState()
    history = MinScalarHistory("loss")
    state.add_history(history)
...
def test_vanilla_engine_state_get_history_does_not_exist():
    state = VanillaEngineState()
...
def test_vanilla_engine_state_get_histories():
...
def test_vanilla_engine_state_get_histories_empty():
    assert VanillaEngineState().get_histories() == {}


def test_vanilla_engine_state_get_module():
    state = VanillaEngineState()
    state.add_module("my_module", nn.Linear(4, 5))
    assert isinstance(state.get_module("my_module"), nn.Linear)


def test_vanilla_engine_state_get_module_missing():
    state = VanillaEngineState()
    with raises(ValueError):
        state.get_module("my_module")


def test_vanilla_engine_state_has_history_true():
    state = VanillaEngineState()
    state.add_history(MinScalarHistory("loss"))
    assert state.has_history("loss")


def test_vanilla_engine_state_has_history_false():
    assert not VanillaEngineState().has_history("loss")


def test_vanilla_engine_state_has_module_true():
    state = VanillaEngineState()
    state.add_module("my_module", nn.Linear(4, 5))
    assert state.has_module("my_module")


def test_vanilla_engine_state_has_module_false():
    assert not VanillaEngineState().has_module("my_module")


def test_vanilla_engine_state_increment_epoch_1():
    state = VanillaEngineState()
    assert state.epoch == -1
...
def test_vanilla_engine_state_increment_epoch_2():
    state = VanillaEngineState()
    assert state.epoch == -1
...
def test_vanilla_engine_state_increment_iteration_1():
    state = VanillaEngineState()
    assert state.iteration == -1
...
def test_vanilla_engine_state_increment_iteration_2():
    state = VanillaEngineState()
    assert state.iteration == -1
...
def test_vanilla_engine_state_load_state_dict():
    state = VanillaEngineState()
...
def test_vanilla_engine_state_state_dict_load_state_dict():
    state = VanillaEngineState()
...
def test_vanilla_engine_state_remove_module_exists():
    state = VanillaEngineState()
    state.add_module("my_module", nn.Linear(4, 5))
    assert state.has_module("my_module")
...
def test_vanilla_engine_state_remove_module_does_not_exist():
    state = VanillaEngineState()
    with raises(ValueError):
        state.remove_module("my_module")


def test_vanilla_engine_state_state_dict():
