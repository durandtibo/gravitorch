...
def test_prepare_quantiles_list_or_tuple(quantiles: Union[list[float], tuple[float, ...]]):
    assert prepare_quantiles(quantiles).equal(torch.tensor([0.2, 0.8], dtype=torch.float))


def test_prepare_quantiles_torch_tensor():
    assert prepare_quantiles(torch.tensor([0.2, 0.8])).equal(
        torch.tensor([0.2, 0.8], dtype=torch.float)
    )


def test_prepare_quantiles_torch_tensor_sort_values():
