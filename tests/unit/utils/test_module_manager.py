...
def test_module_manager_add_module(name: str):
    manager = ModuleManager()
    manager.add_module(name, nn.Linear(4, 5))
    assert name in manager._modules


def test_module_manager_add_module_override():
    manager = ModuleManager()
...
def test_module_manager_get_module():
    manager = ModuleManager()
    manager.add_module("my_module", nn.Linear(4, 5))
    assert isinstance(manager.get_module("my_module"), nn.Linear)


def test_module_manager_get_module_missing():
    manager = ModuleManager()
    with raises(ValueError):
        manager.get_module("my_module")


def test_module_manager_has_module_true():
    manager = ModuleManager()
    manager.add_module("my_module", nn.Linear(4, 5))
    assert manager.has_module("my_module")


def test_module_manager_has_module_false():
    assert not ModuleManager().has_module("my_module")


def test_module_manager_remove_module_exists():
    manager = ModuleManager()
    manager.add_module("my_module", nn.Linear(4, 5))
...
def test_module_manager_remove_module_missing():
    manager = ModuleManager()
    with raises(ValueError):
        manager.remove_module("my_module")


def test_module_manager_state_dict_1_module_with_state_dict_method():
    manager = ModuleManager()
    model = nn.Linear(4, 5)
...
def test_module_manager_state_dict_1_module_without_state_dict_method():
    manager = ModuleManager()
...
def test_module_manager_state_dict_2_modules():
    manager = ModuleManager()
    model = nn.Linear(4, 5)
    model.weight.data.fill_(1.0)
    model.bias.data.fill_(1.0)
...
def test_module_manager_load_state_dict_1_module():
    manager = ModuleManager()
    model = nn.Linear(4, 5)
...
def test_module_manager_load_state_dict_2_modules():
    manager = ModuleManager()
    model = nn.Linear(4, 5)
    model.weight.data.fill_(2.0)
    model.bias.data.fill_(2.0)
...
def test_module_manager_load_state_dict_missing_key_in_state_dict():
    manager = ModuleManager()
    manager.load_state_dict(state_dict={}, keys=(ct.MODEL,))
    assert len(manager) == 0


def test_module_manager_load_state_dict_missing_module():
    manager = ModuleManager()
    manager.load_state_dict(state_dict={ct.MODEL: 123}, keys=(ct.MODEL,))
    assert len(manager) == 0


def test_module_manager_load_state_dict_no_load_state_dict():
    manager = ModuleManager()
